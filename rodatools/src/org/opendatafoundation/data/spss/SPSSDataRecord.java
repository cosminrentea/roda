...
public class SPSSDataRecord {
	...
	// 8-byte cluster for compressed files
	// (this value is retained between
	// calls)
	static byte[] cluster = new byte[8];

	// for compressed files (once initialized,
	// this value is retained between calls)
	static byte clusterIndex = 8;

	/**
	 * Reads the values for the current observation into memory. This assumes
	 * that the file pointer is properly positioned.
	 * 
	 * @throws IOException
	...
	/**
	 * Reads the values for the current observation into memory. If fromDisk is
	 * set to true, the record values are loaded into the variable single values
	 * instead of the data list This assumes that the file pointer is properly
	 * positionned.
	 * 
	 * @throws IOException
	 * @throws SPSSFileException
	 */
	public void read(SPSSFile is, boolean fromDisk) throws IOException,
			SPSSFileException {
		...
		while (varIterator.hasNext()) {
			...
			if (var.type == SPSSVariable.VariableType.NUMERIC) {
				...
				strData = "";
				// string: depends on string length but always in blocks of 8
				// bytes
			...
			while (blocksToRead > 0) {
				...
				if (file.isCompressed()) {
					...
					if (clusterIndex > 7) {
						...
						// need to read a new compression cluster of up to 8
						// variables
						file.read(cluster);
						clusterIndex = 0;
					}
					...
					case 0: // skip this code
						break;
					case 252: // end of file, no more data to follow. This
								// should not happen.
						throw new SPSSFileException(
								"Error reading data: unexpected end of compressed data file (cluster code 252)");
					case 253: // data cannot be compressed, the value follows
								// the cluster
						if (var.type == SPSSVariable.VariableType.NUMERIC) {
							numData = file.readSPSSDouble();
						} else { // STRING
							// read a maximum of 8 characters but could be less
							// if this is the last block
							int blockStringLength = Math.min(8,
									charactersToRead);
							// append to existing value
							strData += file.readSPSSString(blockStringLength);
							// if this is the last block, skip the remaining
							// dummy byte(s) (in the block of 8 bytes)
					...
					case 255: // system missing value
						if (var.type == SPSSVariable.VariableType.NUMERIC) {
							...
							// string variable
							throw new SPSSFileException(
									"Error reading data: unexpected SYSMISS for string variable");
						}
						break;
					default: // 1-251 value is code minus the compression BIAS
								// (normally always equal to 100)
						if (var.type == SPSSVariable.VariableType.NUMERIC) {
							// numeric variable
							numData = byteValue
									- file.infoRecord.compressionBias;
							// file.log(""+numVar.data.get(numVar.data.size()-1));
						} else {
							// string variable
							throw new SPSSFileException(
									"Error reading data: unexpected compression code for string variable");
						}
						break;
					}
				} else {
					...
					if (var.type == SPSSVariable.VariableType.NUMERIC) {
						...
						// read a maximum of 8 characters but could be less if
						// this is the last block
						int blockStringLength = Math.min(8, charactersToRead);
						// append to existing value
						strData += file.readSPSSString(blockStringLength);
						// if this is the last block, skip the remaining dummy
						// byte(s) (in block of 8 bytes)
						if (charactersToRead < 8) {
			...
			if (var.type == SPSSVariable.VariableType.STRING) {
				// If the variable is a string and all the blocks where blank
				// (254), make it an empty string
			 ...
			 * file.log("Numeric variable "
			 * +var.variableRecord.name+" value "+numVar.data.get(
			 * numVar.data.size()-1) ) ; } else {
			 * file.log("String variable "+var
			 * .variableRecord.name+" value ["+strVar.data.get(
			 * strVar.data.size()-1) +"]") ; }
			 */
		} // next variable
	}
