...
privileged aspect UserProfileController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> UserProfileController.showJson(@PathVariable("userId") Integer userId) {
    ...
    public ResponseEntity<String> UserProfileController.deleteFromJson(@PathVariable("userId") Integer userId) {
        UserProfile userProfile = userProfileService.findUserProfile(userId);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        if (userProfile == null) {
