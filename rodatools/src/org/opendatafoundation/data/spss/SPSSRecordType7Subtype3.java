...
public class SPSSRecordType7Subtype3 extends SPSSAbstractRecordType {
	...
	int characterRepresentation; // 1=EBCDIC, 2=7-bit ASCII, 3=8-bit ASCII,
									// 4=DEC Kanji
	...
	public void read(SPSSFile is) throws IOException, SPSSFileException {
		...
		if (recordTypeCode != 7)
			throw new SPSSFileException(
					"Error reading record type 7 subtype 3: bad record type ["
		...
		if (recordSubtypeCode != 3)
			throw new SPSSFileException(
					"Error reading record type 7 subtype 3: bad subrecord type ["
							+ recordSubtypeCode
		...
		if (dataElementLength != 4)
			throw new SPSSFileException(
					"Error reading record type 7 subtype 3: bad data element length ["
							+ dataElementLength + "]. Expecting 4.");

		numberOfDataElements = is.readSPSSInt();
		if (numberOfDataElements != 8)
			throw new SPSSFileException(
					"Error reading record type 7 subtype 3: bad number of data elements ["
	...
	public String toString() {
		...
		str += "\nFloating point  : " + floatRepresentation + " ["
				+ getFloatRepresentationLabel() + "]";
		str += "\nEndianness      : " + endianness + " ["
				+ getEndiannessLabel() + "]";
		;
		str += "\nCharacter set   : " + characterRepresentation + " ["
