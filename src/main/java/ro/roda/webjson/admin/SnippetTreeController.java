...
@RequestMapping("/adminjson/cmssnippettree")
@Controller
public class SnippetTreeController {

	@Autowired
	SnippetTreeService snippetTreeService;
	...
	public ResponseEntity<String> showJson(@PathVariable("id") Integer id) {
		SnippetTree snippetTree = snippetTreeService.findSnippetTree(id);
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		if (snippetTree == null) {
			return new ResponseEntity<String>(headers, HttpStatus.NOT_FOUND);
		}
		return new ResponseEntity<String>(snippetTree.toJson(), headers, HttpStatus.OK);
	}
