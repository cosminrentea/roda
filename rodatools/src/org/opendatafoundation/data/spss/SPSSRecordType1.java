	...
	public void read(SPSSFile is) throws IOException, SPSSFileException {
		...
		if (recordTypeCode.compareTo("$FL2") != 0)
			throw new SPSSFileException(
					"Read header error: this is not a valid SPSS file. Does not start with $FL2.");
		// identification
		productIdentification = is.readSPSSString(60).replaceAll("\\s+$", "");

		// layout code
		long filePointer = is.getFilePointer(); // save position for endianness
												// check below
		layoutCode = is.readSPSSInt();

		// --> layoutCode should be 2 or 3.
		// --> If not swap bytes and check again which would then indicate
		// big-endian
		...
		// check layout type
		/*
		 * 20080303-PH: Removed this check as received valid files with
		 * layout=3. if(layoutCode==3) { throw new SPSSFileException(
		 * "Read header: SPSS Portable files not supported (layout Code ["
		 * +layoutCode+"])."); } if(layoutCode!=2) { throw new
		 * SPSSFileException(
		 * "Read header error: invalid Layout Code ["+layoutCode
		 * +"]. Value should be 2."); }
		 */
		// OBS
