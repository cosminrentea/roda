...
public class AddressController {
	...
	public String update(@Valid Address address, BindingResult bindingResult, Model uiModel,
			HttpServletRequest httpServletRequest) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, address);
	...
	public ResponseEntity<String> jsonFindAddressesByCityId(@RequestParam("cityId") City cityId) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		return new ResponseEntity<String>(Address.toJsonArray(Address.findAddressesByCityId(cityId).getResultList()),
				headers, HttpStatus.OK);
	}

	@RequestMapping(params = "find=ByCityIdAndPostalCodeEquals", headers = "Accept=application/json")
	@ResponseBody
	public ResponseEntity<String> jsonFindAddressesByCityIdAndPostalCodeEquals(@RequestParam("cityId") City cityId,
			@RequestParam("postalCode") String postalCode) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		return new ResponseEntity<String>(Address.toJsonArray(Address.findAddressesByCityIdAndPostalCodeEquals(cityId,
				postalCode).getResultList()), headers, HttpStatus.OK);
	}
	...
	public ResponseEntity<String> jsonFindAddressesByPostalCodeEquals(@RequestParam("postalCode") String postalCode) {
		HttpHeaders headers = new HttpHeaders();
		headers.add("Content-Type", "application/json; charset=utf-8");
		return new ResponseEntity<String>(Address.toJsonArray(Address.findAddressesByPostalCodeEquals(postalCode)
				.getResultList()), headers, HttpStatus.OK);
	}
}
