...
my $newroot = $roda->dbschema->resultset('CmsFolder')->create({name => 'TestRoot', description => 'Acesta nu trebuie sa existe in momentul rularii textului, asa ca, if you see me, you fucked up smth...'});

isa_ok($newroot, 'RODA::RODADB::Result::CmsFolder', 'Verificarea obiectului returnat de create');
...
my $new = $roda->dbschema->resultset('CmsFolder')->create({ name=>'TestChild', description => 'Test child lets see' });

isa_ok($new, 'RODA::RODADB::Result::CmsFolder', 'Verificarea obiectului returnat de create');

$newroot->attach_child($new);
...
isa_ok($children[0], 'RODA::RODADB::Result::CmsFolder', 'Aschia n-a sarit prea departe de trunchi');


cmp_ok($children[0]->name, 'eq', 'TestChild', 'Copilul are numele corect');

isa_ok($children[0]->parent, 'RODA::RODADB::Result::CmsFolder', 'Copilul este recunoscut de parinte');

cmp_ok($children[0]->parent->name, 'eq', 'TestRoot', 'Copilul este recunoscut de parintele corect');


my $juju = $roda->dbschema->resultset('CmsFolder')->create({ name=>'juju', description => 'juju lets see' });

isa_ok($juju, 'RODA::RODADB::Result::CmsFolder', 'Verificarea obiectului returnat de create');
...
isa_ok($juju->parent, 'RODA::RODADB::Result::CmsFolder', 'Parintele seamana la specie cu copilul');
...
my $abel = $roda->dbschema->resultset('CmsFolder')->create({ name=>'Abel', description => 'abel description' });
my $cain = $roda->dbschema->resultset('CmsFolder')->create({ name=>'Cain', description => 'cain description' });
...
print "Daca stergem root-ul principal ".$newroot->name." , trebuie sa dispara toate\n";

$newroot->delete;

my $count = $roda->dbschema->resultset('CmsFolder')->search({})->count;

cmp_ok($count, '==', 1 , 'In tabel a ramas doar root-ul introdus de seeddb');
