...
privileged aspect PersonController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> PersonController.showJson(@PathVariable("id") Integer id) {
    ...
    public ResponseEntity<String> PersonController.deleteFromJson(@PathVariable("id") Integer id) {
        Person person = personService.findPerson(id);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        if (person == null) {
