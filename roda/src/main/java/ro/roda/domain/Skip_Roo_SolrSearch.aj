...
privileged aspect Skip_Roo_SolrSearch {
    ...
    public static void Skip.indexSkips(Collection<Skip> skips) {
        ...
        for (Skip skip : skips) {
            ...
            sid.addField("skip_solrsummary_t", new StringBuilder().append(skip.getVariableId()).append(" ").append(skip.getNextVariableId()).append(" ").append(skip.getCondition()).append(" ").append(skip.getId()));
            documents.add(sid);
        }
        try {
