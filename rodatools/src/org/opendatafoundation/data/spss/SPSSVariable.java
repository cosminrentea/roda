...
public abstract class SPSSVariable {
	...
	// The SPSS type 2 record describing this variable
	@OneToOne(cascade = CascadeType.ALL)
	protected SPSSRecordType2 variableRecord;

	// The optional SPSS type 3
	// record holding this variable value labels
	@OneToOne(cascade = CascadeType.ALL)
	protected SPSSRecordType3 valueLabelRecord;
	...
	protected Map<String, SPSSVariableCategory> categoryMap = new LinkedHashMap<String, SPSSVariableCategory>();
	 ...
	 * @throws DOMException
	 */
	public Element getDDI3CodeScheme(Document doc) throws DOMException,
			SPSSFileException {
	...
	private Element getDDI3CodeScheme(Document doc, String categorySchemeID,
			String codeSchemeID) throws DOMException, SPSSFileException {
		Element scheme = null;
		Element elem;
		...
		if (!this.categoryMap.isEmpty()) {
			...
			Element categorySchemeReference = (Element) scheme.appendChild(doc
					.createElementNS(SPSSFile.DDI3_LOGICAL_PRODUCT_NAMESPACE,
							"CategorySchemeReference"));
			elem = (Element) categorySchemeReference.appendChild(doc
