...
sub initialize {
  ...
  $self->{StartWithNewPage} = 1    # Start new page for pod section
    unless exists $self->{StartWithNewPage};
  $self->{TableOfContents}  = 1    # Add table of contents
...
sub verbatim {
  ...
  if ($self->{_dont_modify_any_para}) {
   ...
   $self->_output('\begin{lstlisting}[language=perl]' . "\n$paragraph\n". '\end{lstlisting}'."\n");
...
sub textblock {
  ...
  if ($self->{_CURRENT_HEAD1} =~ /^NUME/i && $self->ReplaceNAMEwithSection()) {
...
print "in here - ------------------------------\n";
  ...
  # we return immediately if we get it
    ...
    if ($self->{_CURRENT_HEAD1} =~ /^METODE/i ) {
    
        print "in here - ------------------------------\n";
    
       $hline =   "\\begin{center} \\line(1,0){250} \\end{center}";
  ...
  # Section
  $self->_output($hline."\\" .$LatexSections[$level] .$star ."{$paragraph\\label{".$label ."}\\index{".$index."}}\n");
