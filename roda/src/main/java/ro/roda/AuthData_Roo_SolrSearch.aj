...
privileged aspect AuthData_Roo_SolrSearch {
    ...
    public static void AuthData.indexAuthDatas(Collection<AuthData> authdatas) {
        ...
        for (AuthData authData : authdatas) {
            ...
            sid.addField("authData.user_t", authData.getUser());
            ...
            sid.addField("authdata_solrsummary_t", new StringBuilder().append(authData.getUser()).append(" ").append(authData.getCredentialProvider()).append(" ").append(authData.getFieldName()).append(" ").append(authData.getFieldValue()));
            documents.add(sid);
        }
        try {
